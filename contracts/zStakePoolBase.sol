 ...
 */
abstract contract zStakePoolBase is IPool, zStakeAware, ReentrancyGuard {
    ...
    function pendingYieldRewards(address _staker) external view override returns (uint256) {
        ...
        if (blockNumber() > lastYieldDistribution && usersLockingWeight != 0) {
            ...
            uint256 ilvRewards = (multiplier * weight * factory.wildPerBlock()) / factory.totalWeight();
    ...
    function _processRewards(
        ...
        if (_useSILV) {
            ...
            address ilvPool = factory.getPoolAddress(wild);
